...
function NotifIcon({ type }) {
    if (type === 'appointment-status')
        return (
            <span className='grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[var(--tt-canvas)] text-[var(--tt-brand)] border border-[var(--tt-border)]'>
                <CalendarDays size={15} />
            </span>
        )
    return (
        <span className='grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[var(--tt-sage)] text-[var(--tt-brand)] border border-[var(--tt-border)]'>
            <Megaphone size={15} />
        </span>
    )
}
                ...
                >
                    ...
                    <div className='flex items-center justify-between border-b border-[#E8EEE9] bg-[#FBFDFB] px-4 py-3.5 rounded-t-2xl'>
                        <div className='flex items-center gap-2'>
                            <span className='grid h-7 w-7 place-items-center rounded-lg bg-[var(--tt-sage)] text-[var(--tt-brand-strong)]'>
                                <Bell size={14} />
                            </span>
                        ...
                        <div className='flex items-center gap-1'>
                            {unreadCount > 0 && (
                                <button
                                    ...
                                    className='flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-bold text-[var(--tt-brand)] transition hover:bg-[var(--tt-sage)]'
                                >
                                    <CheckCheck size={14} />
                                    Mark all read
                                </button>
                            ...
                            <button
                                ...
                                className='grid h-7 w-7 place-items-center rounded-lg text-[var(--tt-muted)] transition hover:bg-[var(--tt-canvas)] hover:text-[var(--tt-ink)]'
                    ...
                    <div className='max-h-[380px] overflow-y-auto divide-y divide-[#E8EEE9]'>
                        {loading && notifications.length === 0 ? (
                            <div className='flex flex-col items-center justify-center gap-2 py-10 text-[var(--tt-muted)]'>
                                <span className='h-5 w-5 animate-spin rounded-full border-2 border-[var(--tt-brand-strong)] border-t-transparent' />
                            ...
                            <div>
                                {notifications.map((notif) => (
                                    ...
                                    >
                                        ...
                                        {!notif.isRead && (
                                            <span className='mt-1.5 h-2 w-2 shrink-0 rounded-full bg-[var(--tt-brand-strong)]' />
                                        )}
                                    </button>
                                ))}
                            </div>
