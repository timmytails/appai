...
const normalizeAgeMonths = (val) => {
    if (val === undefined || val === null || val === '') return '12+'
    const str = String(val).trim()
    if (str === '12+' || str.toLowerCase() === '12plus' || str.startsWith('12+')) return '12+'
    const num = Number(str)
    if (!isNaN(num)) {
        return num >= 13 ? '12+' : num
    }
    return str
}
...
const validators = [
    ...
    body('ageMonths')
        ...
        .custom((val) => {
            ...
            if (isNaN(num) || num < 0) {
                throw new Error('Age in months must be a number or 12+')
            }
            return true
        }),
...
router.post('/', protect, validators, async (req, res) => {
    ...
    try {
        ...
        const pet = await Pet.create({
            ...
            ageMonths: normalizeAgeMonths(req.body.ageMonths),
