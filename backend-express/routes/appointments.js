    ...
    async (req, res) => {
        ...
        try {
            ...
            if (req.user && req.user._id) {
                userAppointment = await Appointment.findOne({
                    user: req.user._id,
                    date,
                    status: { $in: ACTIVE_BOOKING_STATUSES }
                }).select('time endTime petName service')
            }
            ...
            return res.json({
                ...
                userHasAppointment: Boolean(userAppointment),
                userAppointment: userAppointment
                    ? {
                        time: userAppointment.time,
                        endTime: userAppointment.endTime,
                        petName: userAppointment.petName,
                        service: userAppointment.service
                    }
                    : null,
                slots
            })
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (req.user && req.user._id) {
                ...
                }).select('date time endTime petName service')

                userAppointments.forEach((appt) => {
                    userAppointmentsByDate[appt.date] = {
                        time: appt.time,
            ...
            for (
                ...
                if (date < today) {
                    ...
                    status = 'closed'
                } else if (userAppointmentsByDate[date]) {
                    status = 'user-booked'
                } else {
                    const slots =
                        createSlotRows(
                ...
                dates.push({
                    date,
                    status,
                    userAppointment: userAppointmentsByDate[date] || null
                })
            }
            ...
            return res.json({
                success: true,
                month,
                userBookedDates: userAppointmentsByDate,
                dates
            })
        } catch (error) {
            console.error(
...
router.post(
    ...
    [
        ...
        body('petAgeMonths')
            ...
            .custom((val) => {
                if (val === '12+' || val === '12plus' || val === 12 || (typeof val === 'string' && val.trim() === '12+')) return true
                const num = Number(val)
                if (isNaN(num) || num < 0) {
                    throw new Error('Pet age in months must be a valid number or 12+')
                }
                return true
            }),
        ...
        try {
            ...
            if (existingCustomerAppointment) {
                return res.status(400).json({
                    success: false,
                    message: `You already have an appointment booked on this day (${existingCustomerAppointment.time} for ${existingCustomerAppointment.petName || 'your pet'}). Customers may only book one appointment per day.`
                })
            }
            ...
            const rawAge = pet?.ageMonths !== undefined
                ? pet.ageMonths
                : (req.body.petAgeMonths !== undefined && req.body.petAgeMonths !== '' ? req.body.petAgeMonths : null)
            let authoritativePetAgeMonths = null
            if (rawAge !== null && rawAge !== undefined && rawAge !== '') {
                const strAge = String(rawAge).trim()
                const numAge = Number(strAge)
                if (strAge === '12+' || (!isNaN(numAge) && numAge >= 13)) {
                    authoritativePetAgeMonths = '12+'
                } else if (!isNaN(numAge)) {
                    authoritativePetAgeMonths = numAge
                } else {
                    authoritativePetAgeMonths = strAge
                }
            }
            const authoritativeVaccinated = pet?.vaccinated !== undefined
                ? (pet.vaccinated !== false && pet.vaccinated !== 'no' && pet.vaccinated !== 'false')
                : (req.body.vaccinated === false || req.body.vaccinated === 'false' || req.body.vaccinated === 'no' ? false : true)

            if (authoritativePetAgeMonths !== null && typeof authoritativePetAgeMonths === 'number' && authoritativePetAgeMonths < 3) {
                return res.status(400).json({
                    success: false,
...
router.patch(
    ...
    async (req, res) => {
        ...
        try {
            ...
            // Check if user already has another appointment on that date
            if (!isAdmin && appointment.date !== date) {
                const userConflictOnDate = await Appointment.findOne({
                ...
                if (userConflictOnDate) {
                    return res.status(400).json({
                        success: false,
                        message: `You already have another appointment booked on ${date} (${userConflictOnDate.time}). Customers may only have one appointment per day.`
                    })
                }
            }
            ...
            // Update startAt / endAt ISO objects using Manila time
            const startMinutes = toMinutes(slot.startTime)
            const endMinutes = toMinutes(slot.endTime)
            const startAt = manilaDateTime(date, startMinutes)
            const endAt = manilaDateTime(date, endMinutes)
...
router.put('/:id/reschedule', protect, (req, res, next) => {
    req.method = 'PATCH'
    return router.handle(req, res, next)
})

router.post('/:id/reschedule', protect, (req, res, next) => {
    req.method = 'PATCH'
    return router.handle(req, res, next)
})
