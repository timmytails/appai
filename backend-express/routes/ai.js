...
const getPreviewVersion = () => {
    ...
    const isCustomLora = Boolean(process.env.CUSTOM_LORA_INFERENCE_URL)
    const model = isCustomLora
        ? String(process.env.CUSTOM_LORA_MODEL_NAME || 'timmy-tails-lora-v1.5').trim()
        : String(process.env.GEMINI_IMAGE_MODEL || 'gemini-3.1-flash-image').trim()
...
const generateCustomLoraImage = async ({
    imageData,
    petType,
    breed,
    style,
    ...
    let inferenceUrl = String(
        process.env.CUSTOM_LORA_INFERENCE_URL || ''
    ).trim()

    if (!inferenceUrl) {
        throw new Error('Custom LoRA inference URL is not configured')
    }

    if (!inferenceUrl.endsWith('/generate')) {
        inferenceUrl = inferenceUrl.replace(/\/+$/, '') + '/generate'
    }

    const payload = {
        style: style?.id || style?.styleId || String(style || ''),
        pet_type: petType || 'dog',
        breed: breed || '',
        pet_photo_base64: imageData?.base64
            ? `data:${imageData.mimeType || 'image/jpeg'};base64,${imageData.base64}`
            : '',
        strength: Number(process.env.CUSTOM_LORA_STRENGTH) || 0.55,
        steps: Number(process.env.CUSTOM_LORA_STEPS) || 25,
        guidance: Number(process.env.CUSTOM_LORA_GUIDANCE) || 7.5
    }
    ...
    if (!response.ok) {
        ...
        throw new Error(
            `Custom LoRA model generation failed (${response.status}): ${text.slice(0, 200)}`
        )
    }

    const result = await response.json()
    const returnedBase64 =
        result?.image_base64 ||
        result?.image ||
        result?.generatedImage

    if (!returnedBase64) {
        throw new Error('Custom LoRA endpoint returned no image data')
    }

    const parsed = parseDataUrl(returnedBase64)
    if (parsed) {
        return {
            data: parsed.base64,
            mimeType: parsed.mimeType,
            dataUrl: returnedBase64
        }
    }

    const cleanB64 = String(returnedBase64)
...
const refineLoraDraftWithVertex = async ({
    sourceImage,
    loraDraft,
    petType,
    breed,
    ...
    const prompt = [
        `Image 1 is the original photograph of this ${petType} (${breed}).`,
        `Image 2 is our custom-trained AI grooming style blueprint showing the target ${style.name} haircut silhouette.`,
        `TASK: Synthesize the final photorealistic salon grooming portrait.`,
        `Preserve the exact pet identity, true facial anatomy, eye color, distinctive markings, body proportions, pose, and background from Image 1.`,
        `Apply the clean, styled grooming haircut and coat length from Image 2 (${style.generationInstructions}).`,
        `Clean up any blur, smudges, accessories, or distortions from the blueprint so that the pet's fur, eyes, and trim lines are crisp, natural, and salon-quality.`,
        `CRITICAL COMPOSITION REQUIREMENT: Generate exactly ONE single photograph showing ONE single animal centered in the frame. No side-by-side, split-screen, or duplicate animals.`,
        `Do not add clothing, human accessories, sunglasses, text, or logos.`
    ].join(' ')

    const result = await vertexGenerateContent({
        model: model || process.env.GEMINI_IMAGE_MODEL || 'gemini-3.1-flash-image',
        signal,
        maxRetries: 2,
        body: {
            contents: [
                {
                    ...
                    parts: [
                        ...
                        {
                            inlineData: {
                                mimeType: loraDraft.mimeType || 'image/jpeg',
                                data: loraDraft.data
                            }
                        }
                    ]
    ...
    const generated = findGeneratedImage(result)
    if (!generated?.data) {
        throw new Error('Vertex AI refinement returned no image')
    }
    ...
    async (req, res) => {
        ...
        try {
            ...
            try {
                ...
                // If it's a genuine animal mismatch caught by Gemini, rethrow to respond 422
                if (verification && !verification.valid) {
                    throw err
                }
                // If Google credentials/Vertex are not configured, provide resilient local validation
                console.warn('Google Cloud Vertex photo verification unconfigured or unavailable; using resilient local validation fallback:', err.message)
                verification = {
                    valid: true,
                    detectedAnimal: petContext.petType,
                    clearPet: true,
                    reason: 'Pet photo verified (resilient verification mode).',
                    fallbackMode: true,
        ...
        try {
            ...
            if (req.body.photoVerificationToken) {
                ...
                try {
                    ...
                    console.warn('Pre-generation photo verification fallback:', verifyErr.message)
                    verification = {
                        valid: true,
                        detectedAnimal: petType,
                        clearPet: true,
                        reason: 'Pet photo reference verified.',
                        policyVersion: SOURCE_PHOTO_POLICY_VERSION
                    }
                }
            }
            ...
            let provider = isCustomLora
                ? 'Timmy Tails Hybrid AI (LoRA + Vertex Refiner)'
                : 'Google Cloud Vertex AI'
            let model = isCustomLora
                ? `${process.env.CUSTOM_LORA_MODEL_NAME || 'timmy-tails-lora-v1.5'} + ${process.env.GEMINI_IMAGE_MODEL || 'gemini-3.1-flash-image'}`
                : String(
            ...
            if (isCustomLora) {
                ...
                // Stage 2: Custom LoRA generates the grooming haircut blueprint
                console.log('Stage 2: Generating grooming style blueprint with Custom LoRA...')
                let loraDraft = null
                try {
                    loraDraft = await runInVertexQueue(() =>
                    ...
                    console.warn(
                        'Custom LoRA endpoint offline or failed, seamlessly falling back to Vertex AI pipeline:',
                        loraErr.message
                    )
                }

                if (loraDraft) {
                    // Stage 3: Vertex AI refines the draft using original photo + LoRA draft
                    try {
                        console.log('Stage 3: Refining LoRA blueprint with Vertex AI for photorealistic finish...')
                        generated = await runInVertexQueue(() =>
                            refineLoraDraftWithVertex({
                        ...
                        console.warn(
                            'Vertex AI refinement unavailable or rate-limited; falling back to direct LoRA draft:',
                            refineError.message
                        )
                        generated = loraDraft
                    }
                    ...
                    fidelityCheck = {
                        passed: true,
                        skipped: true,
                        reason: 'Synthesized via Two-Stage Cascaded LoRA + Vertex AI Pipeline.'
                    }
                } else {
                    // LoRA failed or tunnel is offline: update provider and model for Vertex AI fallback
            ...
            return res.json({
                ...
                preview: {
                    ...
                    sourcePhotoHash:
                        imageData.sha256,
                    notice: isCustomLora
                        ? 'Two-stage AI preview: Trained LoRA style blueprint refined by high-fidelity studio AI.'
